...
public class RequestAppProcess {
	...
	public static void main(String[] args) throws IOException, IllegalStateException, URISyntaxException, JSONException {
		...
		// suppress log4j messages from UCD library
				...
				JSONObject obj2=(JSONObject) JSONValue.parse(jsonString2);
				JSONArray compArr=(JSONArray) obj.get("COMPONENTS");
				JSONArray envArr= (JSONArray) obj2.get("ENVIRONEMNT");
	...
	private static String getComponentVersionProp(String username,
			String password, String baseUrl, String componentName,String environment,String appName)
		...
		HttpGet getReq = new HttpGet(new URI(baseUrl
				+ "/cli/environment/componentProperties?environment="+environment+"&application="+appName+"&component="+componentName));
		...
		JSONArray envProps = (JSONArray) JSONValue.parse(json);
		//ArrayList<String> versionList = new ArrayList<String>();
		String latest_Version=null;
		// Iterates the JSON Array
		for (int i = 0; i < envProps.size(); i++) {
			JSONObject objects = (JSONObject) envProps.get(i);
			// Gets the object named "name" and stores it in a string
			String name = objects.get("name").toString();
			if(name.equalsIgnoreCase("Latest_Version")){
				latest_Version=	objects.get("value").toString();
			//versionList.add(name);
			}
		}
		//String latestVersion = versionList.get(0);
		System.out.println("Latest version of "+componentName+" for environment "+environment+": " + latest_Version);
