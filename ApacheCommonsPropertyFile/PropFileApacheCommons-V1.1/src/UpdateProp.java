...
public class UpdateProp {
	...
	private static void updateFile(String username, String password,
			...
			String versionNo, String dir,String operation) throws IOException, ConfigurationException{
		...
		if(operation.equalsIgnoreCase("New Configuration")){
			...
			configuration.getLayout().setComment(prop_key, "This property is added by Cloudset job.");
		}
		else if(operation.equalsIgnoreCase("Update Configuration")){
			configuration.setProperty(prop_key, prop_value);
			configuration.getLayout().setComment(prop_key, "This property is modified by Cloudset job.");
